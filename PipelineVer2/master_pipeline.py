...
Return your analysis STRICTLY as a JSON object containing:
1. "required_ppe": A list of strings of the most critical Personal Protective Equipment (PPE) required for this specific scenario (e.g., ["hard hat", "safety goggles", "high-visibility vest", "gloves", "safety boots"]).
2. "zones": A list of objects. For every zone you find, you must provide:
...
- is_flat_painted_boundary: boolean. True if the boundary you just described is just painted floor lines or flat tape (no physical 3D objects). False if the boundary consists of physical 3D objects standing on the floor (like cones, hurdles, or barricades). You MUST decide this based on the boundary phrases you just generated!
...
def run_segmentation(image_path):
    ...
    try:
        ...
        json_out_path = os.path.join("PipelineVer2", "outputs", f"qwen_master_output_{os.path.basename(image_path)}.json")
    ...
    for idx, zone in enumerate(valid_zones):
        ...
        is_flat = zone.get("is_flat_painted_boundary", False)
        
        print(f"\n[{zone_id}] is_flat_painted_boundary: {is_flat}")
        ...
        print(f"Processing {zone_id}: Confined BBox [{y1}:{y2}, {x1}:{x2}]")
        ...
        for obj in boundary_objs:
            ...
            for det in detections:
                ...
                zone_sam_detections.append(f"Boundary: '{label}'")
            ...
            if found_valid_mask:
                ...
                print(f" -> No valid overlapping masks found for boundary inside the bounding box.")
        ...
        if boundary_masks_to_apply:
            ...
            if is_flat:
                ...
                # Use base extraction logic (Pipeline 2)
                kernel_size = 11
                ...
                if len(base_points) > 2:
                    ...
                    interior_clip = np.zeros(img.shape[:2], dtype=np.uint8)
                    cv2.fillConvexPoly(interior_clip, shrunk_hull.astype(np.int32), 255)
                    print(f" -> Built shrunk convex hull using {len(base_points)} ground points (Pipeline 2: 3D Objects logic)")
        ...
        for obj in floor_objs:
            ...
            for det in detections:
                ...
                zone_sam_detections.append(f"Floor: '{label}'")
                
            if found_valid_mask:
                print(f" -> Best mask applied for floor was '{best_label}' with score {highest_valid_score:.2f}")
            else:
                print(f" -> No valid overlapping masks found for floor inside the bounding box.")

        print(f"\n   📊 SAM3 Detection Summary for {zone_id}:")
        print(f"   Total valid objects detected: {len(zone_sam_detections)}")
        from collections import Counter
        for obj_label, count in Counter(zone_sam_detections).items():
            print(f"      - {count}x {obj_label}")
    ...
    # Stamp boundary pixels as a nearly-black magic color (0, 0, 1) directly (no alpha)
    # This prevents the tracker from confusing natural black pixels (shadows, shoes) with the boundary!
    img[boundary_combined] = (0, 0, 1)
    
    return img, boundary_combined, blended_mask, required_ppe
...
def main():
    ...
    img, boundary_combined, floor_mask, required_ppe = run_segmentation(args.image)
    
    if img is not None:
        out_name = f"master_output_{os.path.basename(args.image)}"
        out_path = os.path.join("PipelineVer2", "outputs", out_name)
        os.makedirs(os.path.dirname(out_path), exist_ok=True)
        cv2.imwrite(out_path, img)
        
        restricted_area_uint8 = np.zeros(img.shape[:2], dtype=np.uint8)
        restricted_area_uint8[boundary_combined > 0] = 255 # Just to save mask for debugging
        mask_name = f"mask_{out_name}"
        mask_path = os.path.join("PipelineVer2", "outputs", mask_name)
        cv2.imwrite(mask_path, restricted_area_uint8)
        
        print(f"\n✅ Master Pipeline complete! Saved to {out_path}")
        print(f"✅ Saved Hazard Mask to {mask_path}")
